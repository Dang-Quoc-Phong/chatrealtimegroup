...
.chats-right .message-content {
    background: #e1ffc7; /* Background color for the sender's messages */
}

.chats-left .message-content {
    background: #f1f1f1; /* Background color for the recipient's messages */
}
                            ...
                            <div class="select-group-chat">
                                <div class="dropdown">
                                    <a href="javascript:;" class="dropdown-toggle" data-bs-toggle="dropdown">
                                        All Group<i class="bx bx-chevron-down"></i>
                                    </a>
                                    <ul class="dropdown-menu">
                                        <li><a class="dropdown-item" href="group.html">All Group</a></li>
                                        <li><a class="dropdown-item" href="javascript:;">Archive Group</a></li>
                                        <li><a class="dropdown-item" href="javascript:;">Pinned Group</a></li>
                                    </ul>
                                </div>
                            </div>
                ...
                <div>
                                ...
                                <li class="list-inline-item">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-end">
                                        <a href="#" class="dropdown-item "><span><i
                                                    class="bx bx-x"></i></span>Close Group </a>
                                        ...
                                        <a href="#" class="dropdown-item" data-bs-toggle="modal"
                                            data-bs-target="#change-chat"><span><i
                                                    class="bx bx-trash"></i></span>Delete Group</a>
                                        <a href="#" class="dropdown-item" data-bs-toggle="modal"
                    ...
                    <div class="chat-body chat-page-group slimscroll">
                        <div class="messages" id="messages">
                            <!-- Tin nhắn từ người gửi (chats-right) -->
                            <div class="chats chats-right">
                             
                              
                            </div>
                    
                            <!-- Tin nhắn từ người nhận (chats-left) -->
                            <div class="chats chats-left">
        ...
        Echo.join('chat')
            .here(users => {
                users.forEach(item => {
                    ...
                    elemenStatus.classList.add('chat-profile-icon');
                    if (el) {
            ...
            .leaving(user => {
                let el = document.querySelector(`#link_${user.id}`);
                let elemenStatus = el.querySelector('.chat-profile-icon');
                if (elemenStatus) {
            ...
            .listen('UserOnlined', event => {
                ...
                elementChat.classList.add('message-item');
    
                // Check if the sender is the current user
                if (event.user.id === "{{ Auth::user()->id }}") {
                    elementChat.classList.add('message-item-right');
                } else {
                    elementChat.classList.add('message-item-left');
                }
    
                elementChat.textContent = `${event.user.name}: ${event.message}`;
                if (event.user.role === 'leader') {
                    chatProfileName.textContent = "Nhóm trưởng: ";
                    chatProfileName.classList.add('leader-badge');
                    elementChat.appendChild(chatProfileName);
                }
    ...
    <script type="module">
        Echo.private('chat.group.{{ $groupChat->id }}')
            .listen('ChatGroup', event => {
                ...
                messageItem.classList.add('chats');
    
                // Check if the sender is the current user
                if (event.userSend.id == "{{ Auth::user()->id }}") {
                    messageItem.classList.add('chats-right');
                } else {
                    messageItem.classList.add('chats-left');
                }
    
                let chatAvatar = document.createElement('div');
                chatAvatar.classList.add('chat-avatar');
                chatAvatar.innerHTML = `<img src="${event.userSend.image}" class="rounded-circle dreams_chat" alt="image">`;
                ...
                chatProfileName.innerHTML = `<h6>${event.userSend.name}</h6>`;
                if (event.groupChat.leader == event.userSend.id) {
                    let leaderBadge = document.createElement('span');
                    leaderBadge.textContent = "-Nhóm trưởng";
                    leaderBadge.classList.add('leader-badge');
                    chatProfileName.appendChild(leaderBadge);
                }
                ...
                chatContent.appendChild(chatProfileName);
                chatContent.appendChild(messageText);
    
                messageItem.appendChild(chatAvatar);
                messageItem.appendChild(chatContent);
